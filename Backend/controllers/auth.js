...
const transport = nodemailer.createTransport(sendgridTransport({
    auth: { api_key: "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" }
}));
...
exports.signup = async (req, res, next) => {
    ...
    try {
        ...
        const user = new User({ firstName: firstName, lastName: lastName, email: email, password: hashedPassword, gender: gender, dateOfBirth: dateOfBirth, ProfilePic: ProfilePic })
...
exports.verifyCode = async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'valid code', userId: user._id.toString() });
    }
    catch (err) {
        if (!err.statusCode) {
            err.statusCode = 500;
...
exports.setNewPassword = async (req, res, next) => {
    ...
    try {
        ...
        await user.save();
        res.status(200).json({ message: 'password reset successfully', userId: user._id.toString() });
    }
    catch (err) {
        if (!err.statusCode) {
            err.statusCode = 500;
